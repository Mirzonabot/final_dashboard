...
jobs_salary_main = pd.read_sql("SELECT job_title, min_salary, max_salary FROM jobs",con)
jobs_salary_main.drop(index = 0, axis = 0, inplace = True)
jobs_salary_main["difference"] = jobs_salary_main["max_salary"] - jobs_salary_main["min_salary"]
...
def update_output(value1,value2):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=jobs_salary["difference"],y=jobs_salary["job_title"],orientation="h"))
